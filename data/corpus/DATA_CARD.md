...
**Data version:** 2.0.73  
**Generated:** 2026-07-20T12:06:15.675Z  
...
|------|----------|------|-------------|
| entries.jsonl | 924 | 4.43 MB | Rich structured record for every lexicon entry. |
| instructions.jsonl | 8,505 | 3.77 MB | Scholarly question/answer pairs (Phase 1). |
| instructions-train.jsonl | 40,935 | 24.57 MB | Training split (80%) of scholarly + safety examples. |
| eval.jsonl | 10,280 | 6.18 MB | Held-out evaluation split (20%) of scholarly + safety examples. |
...
| oracle-examples.jsonl | 1,921 | 2.19 MB | Conversational Oracle training examples with system/user/assistant turns (Phase 11). |
| symbolic-correspondences.jsonl | 1,233 | 740.0 KB | Symbolic and hermetic correspondences with confidence and provenance (Phase 12). |
...
| chat-train.jsonl | 58,712 | 79.57 MB | Unified chat-format training corpus, 80% deterministic split (Phase 14). |
| chat-eval.jsonl | 14,554 | 19.68 MB | Held-out chat-format evaluation split, 20% (Phase 14). |
| MODEL_CARD.md | 78 | 4.4 KB | Model card with training recipe, evaluation plan, and hardware guidance (Phases 14-15). |
| pretrain.jsonl | 5,491 | 4.03 MB | Raw-text continual pretraining corpus, 95% split (Phase 15). |
| pretrain-validation.jsonl | 292 | 213.3 KB | Held-out raw-text validation split, 5% (Phase 15). |
| huggingface/train.jsonl | 5,491 | 3.30 MB | HuggingFace-compatible continual pretraining split (Phase 15). |
| huggingface/validation.jsonl | 292 | 173.1 KB | HuggingFace-compatible validation split (Phase 15). |
