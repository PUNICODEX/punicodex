...
function assert(condition, message) {
    if (condition) { pass++; } else { fail++; console.log(`  ${C.red}✗${C.reset} ${message}`); }
}

function warnIf(condition, message) {
    if (condition) { warn++; console.log(`  ${C.yellow}⚠${C.reset} ${message}`); }
}
...
LEXICON.forEach(entry => {
    entry.breakdown.forEach((step, i) => {
        ...
        const hasStress = /[\u0301\u0300\u0302\u0304]/.test(nfd); // acute, grave, circumflex, tilde-as-circumflex
        const hasLength = /[\u0304]/.test(nfd);
        ...
        if (step.type === 'dual') {
            const valid = isPrecomposedDual || (hasAcute && hasMacron) || (hasCircumflex && hasMacron) || (hasGrave && hasMacron);
            warnIf(!valid,
                `[${entry.id}] breakdown[${i}] marked "dual" but "${step.to}" lacks both stress+length`);
        }
        if (step.type === 'stress') {
            const valid = isPrecomposedStress || hasAcute || hasGrave || hasCircumflex ||
...
SPOT_CHECKS.forEach(check => {
    ...
    assert(entry.greek === check.greek,
        `[${check.id}] greek is "${entry.greek}" expected "${check.greek}"`);
    assert(entry.unicode === check.expected,
...
LEXICON.forEach(entry => {
    ...
    const unicodeStripped = unicodeLower.replace(/[\u0300-\u036F\u1AB0-\u1AFF\u1DC0-\u1DFF\u20D0-\u20FF\uFE20-\uFE2F]/g, '');
    ...
    if (ascii !== unicodeStripped) {
        ...
        const ANGLICIZED_EXCEPTIONS = new Set([
            'jason','philoctetes','patroclus','ajax','antilochus','autolycus',
            'atalanta','castor','polydeuces','hector','telemachus','alcmene',
            'admetus','icarus','daedalus','eurydice','oceanus','calypso','circe',
            'scylla','europa','ganymede','narcissus','sisyphus','tantalus',
            'protesilaus','laocoon','andromeda','cassiopeia','cepheus','coeus',
            'iapetus','phoebe','erebus','aether','menelaus','cadmus','aegeus',
            'hippolytus','calliope','clotho','cerberus','pegasus','phoenix',
            'griffin','hubris','cocytus','asclepius','paean','hymenaeus',
            'eurypylus','neoptolemus','calchas','cassandra','hecuba','polyxena',
            'jocasta','agave','aetes','nemeanlion','aeolus','eurus','phthonus',
            'alecto','megaera','hades','hephaistos','hermes','hera','poseidon',
            'demeter','prometheus','persephone','artemis','aphrodite','dionysos',
            'athena','ares','nike','hekate','medousa','atlas','zeus','apollon',
            'hestia','nestor','megara','ixion','tartaros','chaos','isis',
            'hathor','eggther','viracocha'
        ]);
...
LEXICON.filter(e => e.pantheon === 'greek' || e.pantheon === 'greek-location').forEach(entry => {
