...
function generateTempleHTML(entry, related) {
    ...
    // Dual-tier: compute secondary variant (strip acute)
    let tier2Unicode = null;
    let tier2Punycode = null;
    if (isDual) {
        tier2Unicode = stripAcute(entry.unicode);
        tier2Punycode = getPunycode(tier2Unicode);
    }
            ...
            <div class="hero-text">
                ...
                <div class="hero-meta reveal-up">
                    ...
                    <div class="domain-bridge">
                        ...
                        <span class="meta-domain-alt">${tier2Unicode.toLowerCase()}.com</span>
                    </div>` : `
        ...
        <div class="container">
            ...
            <div class="name-grid">
                ...
                <div class="name-card reveal-up" data-delay="200">
                    ...
                    <p class="card-unicode">${escapeHtml(entry.unicode)}${isDual && tier2Unicode ? ` <span style="color:var(--white-dim);font-size:1.5rem;">·</span> ${escapeHtml(tier2Unicode)}` : ''}</p>
                    <p class="card-body">The Unicode restoration recovers what ASCII destroyed.${isDual ? ` <strong>Tier-1</strong> preserves the acute stress. <strong>Tier-2</strong> keeps the macron length. Both are attested scholarly orthographies.` : ` This is <strong>philological accuracy</strong> — not decoration. The domain encodes to Punycode, but the browser displays the truth.`}</p>
                </div>
            </div>
            ...
            <div class="punycode-explainer reveal-up">
                ...
                <div class="explainer-box">
                    ${isDual ? `
                    <code class="explainer-code">${entry.unicode.toLowerCase()}.com &rarr; ${punycode || 'xn--...'}<br>${tier2Unicode.toLowerCase()}.com &rarr; ${tier2Punycode || 'xn--...'}</code>
                    <p class="explainer-note">Both forms encode their non-ASCII characters. To the DNS, they are different Punycode strings. To humanity, they are both <em>${escapeHtml(entry.unicode)}</em>.</p>
                    ` : `
        ...
        <div class="container">
            ...
            <div class="sources-section reveal-up">
                <div class="sources-list">
                    ${entry.sources.map(src => `<span class="source-badge">${escapeHtml(src)}</span>`).join('')}
                </div>
            </div>

            ${entry.meaning ? `
            <div class="tier-explanation reveal-up" style="margin-top:3rem;">
                <p class="lead-text" style="font-size:1.1rem;"><strong>Etymology:</strong> ${escapeHtml(entry.meaning)}</p>
            </div>
            ` : ''}
        </div>
    </section>
...
function main() {
    ...
    for (const entry of LEXICON) {
        ...
        // Skip if directory already exists (flagship)
