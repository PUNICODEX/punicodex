 ...
 * production. The copy is idempotent: the destination directory is removed
 * first, then recreated from the canonical source.
 *
 * Run: node scripts/sync-scholars-portal.js
 */
...
const SOURCE = path.join(ROOT, 'platform', 'public', 'scholars');
const DESTINATION = path.join(ROOT, 'scholars');

function syncScholarsPortal() {
  if (!fs.existsSync(SOURCE)) {
    console.error(`✗ Scholars source directory not found: ${SOURCE}`);
    process.exit(1);
  }
  ...
  console.log(`✓ Scholars portal synced to ${path.relative(ROOT, DESTINATION)}/`);
