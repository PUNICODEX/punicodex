...
console.log('  - Created/updated indexed_sites table (with spam_score, quality_score, pagerank, authority_score)');
...
const flagships = [
  ...
  { id: 'ab', domain: 'Ꜣb.com', punycode: 'xn--b-xw3e.com', pantheon: 'egyptian', tier: '2', tier_label: 'Tier-2' },
...
for (const f of flagships) {
  ...
  insertSite.run(
    ...
    f.tier,
    f.tier_label,
    title, desc, title, desc, desc,
    title, desc, 'website', 'PUNYCODEX',
    'en', 1, desc.split(/\s+/).length, desc.length,
    0, 'index,follow', `https://${f.punycode}`
  );
}
...
console.log(`FTS indexed sites: ${db.prepare('SELECT COUNT(*) as c FROM indexed_sites_fts').get().c}`);
