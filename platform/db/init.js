...
// Flagship IDs — derived from the canonical archetype list (js/archetypes-v2.js)
// so db-init can never drift from the flywheel's flagship roster.
...
// Run patron migration after DB is initialized
try {
  const { execSync } = require('node:child_process');
  execSync(`node "${path.join(__dirname, 'migrate-patrons.js')}"`, { stdio: 'inherit' });
} catch (err) {
  console.error('Patron migration failed:', err.message);
  process.exit(1);
}
