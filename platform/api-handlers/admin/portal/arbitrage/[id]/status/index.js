/**
 * POST /api/admin/portal/arbitrage/:id/status
 *
 * Triage flow for an arbitrage request: pending → contacted → closed
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    return res.json(await getPortalService().setArbitrageRequestStatus(id, status, auth));
  } catch (err) {
    sendError(res, err);
  }
};
