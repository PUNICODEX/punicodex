...
- `platform/blog/content/{id}.json` — One canonical blog post per built
  flagship temple. Generated by `scripts/generate-blog-content.js`; edit only
  the JSON if a post needs correction, then rerun the generator.
...
- `platform/api/cards.json` and `game/cards.json` (generated by
  `scripts/generate-cards.js` from the canonical lexicon, archetypes,
  original scripts, and lore catalog)
- `sites/{id}/blog/index.html` — Flagship blog tab pages, generated by
  `scripts/generate-blog-pages.js` from `platform/blog/content/{id}.json` and
...
4. `scripts/convert-images-to-webp.js` — converts missing/outdated raster images to WebP and updates `<picture>` markup
5. `scripts/build-android-assets.js`
6. `scripts/export-platform-lexicon.js`
7. `scripts/build-original-script-lookup.js`
8. `scripts/export-lore-catalog.js`
9. `scripts/generate-cards.js`
10. `scripts/sync-middleware-domains.js`
11. `scripts/sync-public-copy.js`
12. `scripts/generate-temples.js`
13. `scripts/gen-sitemap.js`
14. `scripts/inject-analytics.js`
15. `scripts/update-data-version.js`
16. `scripts/generate-scholars-taxonomy.js`
17. `scripts/generate-scholars-content.js`
18. `scripts/generate-scholars-manifests.js`
19. `scripts/generate-scholars.js`
20. `scripts/generate-blog-content.js`
21. `scripts/generate-blog-pages.js`
...
**Blog tab navigation.** The Blog tab is wired into the standard flagship tab navigation in `templates/flagship/index.html`, `lore/index.html`, `gallery/index.html`, `patterns/index.html`, `scholars/index.html`, `creatives/index.html`, and `patron/index.html`. The extended-lore template (`templates/flagship/lore/extended/index.html`) uses a dedicated on-page anchor nav and does not include the standard tab set; the advertiser dashboard template (`templates/flagship/dashboard.html`) has no tab navigation at all. Those two auxiliary templates intentionally do not carry a Blog tab link.
...
Test suites run via `node test/run-all.js`:
...
5. **Lighthouse Thresholds** (`test/lighthouse.test.js`) — Asserts 90/90/90/90 scores on the 18-page desktop sample. Reports live in `docs/lighthouse/`.
