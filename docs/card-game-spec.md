...
> **Implementation note (v1):** Cards are generated by `scripts/generate-cards.js`
> from the canonical lexicon, archetypes, original scripts, and lore catalog
> into `game/cards.json` + `platform/api/cards.json` (byte-identical,
> deterministic). Rarity mapping: flagship → legendary, dual-tier → epic,
> Tier-1 → rare, Tier-2 notable-domain → uncommon, else common; mythic =
> original-script foils (flagships with a real original script only). Battle
> engine: `game/engine.js` (pure UMD, seeded RNG, full ability effect-DSL;
> stat normalization ÷12 at deck-build time). API: `GET /api/v1/cards`,
> `GET /api/v1/cards/{id}`. Tests: `test/cards.test.js` (set integrity +
> determinism), `test/card-engine.test.js` (rules + 200-game balance sim),
> `test/cards-api.test.js` (API contract).
...
| Legendary | Flagship / owned-domain entries | Limited editions for names PUNICODEX owns |
...
### Domain-Owner Perks
- Owners of PUNICODEX flagship domains receive a unique **Legendary** variant of that name's card.
