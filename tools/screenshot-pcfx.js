/**
 * One-off: screenshot the four PC/FX scene pages at 1400x950, default and
 * reduced-motion, against a local static server. Output: tools/pcfx-shots/.
 * Run: node tools/screenshot-pcfx.js [page ...]
 */
...
const ALL_PAGES = ['pantheon', 'codex', 'lexicon', 'connections'];
  ...
  try {
    for (const name of pages) {
      for (const reduced of [false, true]) {
        ...
        await page.evaluate(() => {
          const el = document.querySelector('.pc-fx-stage') || document.querySelector('canvas[class*="pc-fx"]');
          if (el) el.scrollIntoView({ block: 'center', behavior: 'instant' });
        });
        await page.waitForTimeout(reduced ? 1400 : 3800);
        const shot = path.join(OUT, `${name}-${reduced ? 'reduced' : 'default'}.png`);
