/**
 * PuniCodex — Edge function sample tests
 *
...
test('edge handler rejects missing input', async () => {
  const handler = await loadHandler();
  const res = await handler(makeRequest('https://punicodex.com/api/edge/authenticity/check'));
...
test('edge handler sets cache-related headers', async () => {
  ...
  const res = await handler(
    makeRequest('https://punicodex.com/api/edge/authenticity/check?input=zeus&type=term')
  );
  ...
  assert.ok(headers.has('X-Punicodex-Edge-Model-Version'), 'model version header present');
});
...
test('cache key is deterministic across calls', async () => {
  ...
  const res1 = await handler(
    makeRequest('https://punicodex.com/api/edge/authenticity/check?input=Áres&type=term')
  );
  const res2 = await handler(
    makeRequest('https://punicodex.com/api/edge/authenticity/check?input=áres&type=term')
  );
  assert.strictEqual(
    res1.headers.get('X-Punicodex-Edge-Cache-Key'),
    res2.headers.get('X-Punicodex-Edge-Cache-Key'),
