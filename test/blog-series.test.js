...
test('every built flagship has a series post with a unique title and description', () => {
  ...
  for (const id of BUILT_IDS) {
    ...
    assert.ok(post.description.length > 80 && post.description.length < 220, `${id}: description length ${post.description.length}`);
    ...
    assert.ok(post.seriesNo >= 1 && post.seriesNo <= BUILT_IDS.length, `${id}: series number out of range`);
    titles.add(post.title);
    descriptions.add(post.description);
  }
  assert.strictEqual(titles.size, BUILT_IDS.length, 'series titles must be unique across the fleet');
  assert.strictEqual(descriptions.size, BUILT_IDS.length, 'series descriptions must be unique across the fleet');
});
...
test('series pages render with correct canonical, OG, Article schema, and no placeholders', () => {
  for (const id of BUILT_IDS) {
    ...
    assert.ok(html.includes(`rel="canonical" href="https://punicodex.com/sites/${id}/blog/restoration/"`), `${id}: wrong canonical`);
    assert.ok(html.includes(`"url": "https://punicodex.com/sites/${id}/blog/restoration/"`), `${id}: Article schema url wrong`);
...
test('founding posts cross-link their series file; series posts link back', () => {
  for (const id of BUILT_IDS.slice(0, 25)) {
    ...
    assert.ok(founding.includes('blog-series-nav'), `${id}: founding post missing series cross-link`);
    assert.ok(founding.includes('href="./restoration/"'), `${id}: founding cross-link href wrong`);
    const series = fs.readFileSync(path.join(ROOT, 'sites', id, 'blog', 'restoration', 'index.html'), 'utf8');
  ...
  for (let i = 1; i < BUILT_IDS.length - 1; i++) {
    const html = fs.readFileSync(path.join(ROOT, 'sites', BUILT_IDS[i], 'blog', 'restoration', 'index.html'), 'utf8');
    assert.ok(html.includes(`../../../${BUILT_IDS[i - 1]}/blog/restoration/`), `${BUILT_IDS[i]}: prev link missing`);
    assert.ok(html.includes(`../../../${BUILT_IDS[i + 1]}/blog/restoration/`), `${BUILT_IDS[i]}: next link missing`);
  }
});
...
test('sitemap covers the series; generators are registered in the flywheel', () => {
  ...
  for (const id of BUILT_IDS.slice(0, 40)) {
    assert.ok(sitemap.includes(`/sites/${id}/blog/restoration/`), `sitemap missing ${id} series URL`);
  }
  const gen = fs.readFileSync(path.join(ROOT, 'scripts', 'generate.js'), 'utf8');
  assert.ok(gen.includes('generate-blog-series-restoration.js'), 'series content generator not registered');
  ...
  assert.ok(seriesIdx < pagesIdx && pagesIdx < indexIdx, 'generator order: content → pages → index');
});
...
test('every built flagship has a resonance post connecting myths to industries', () => {
  ...
  for (const id of BUILT_IDS) {
    ...
    if (seats.length > 0) {
      ...
      assert.ok(post.body.includes(topSeat.name), `${id}: top seat ${topSeat.name} absent from body`);
...
test('resonance pages render with correct canonical and series chain', () => {
  ...
  for (let i = 1; i < BUILT_IDS.length - 1; i += 17) {
    ...
    assert.ok(html.includes(`../../../${BUILT_IDS[i - 1]}/blog/resonance/`), `${BUILT_IDS[i]}: prev link missing`);
    assert.ok(html.includes(`../../../${BUILT_IDS[i + 1]}/blog/resonance/`), `${BUILT_IDS[i]}: next link missing`);
  }
});
