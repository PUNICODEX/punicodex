...
(function() {
    'use strict';
    const canvas = document.getElementById('otherworld-canvas');
    const ctx = canvas ? canvas.getContext('2d') : null;
    if (!ctx) return;
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    function hexToRgb(hex){const n=parseInt(hex.replace('#',''),16);return{r:(n>>16)&255,g:(n>>8)&255,b:n&255};}
    function readColor(attr, fb){const v=canvas.getAttribute(attr);return v&&v.startsWith('#')?hexToRgb(v):hexToRgb(fb);}
    const P = readColor('data-primary', '#B8A8D8');
    const S = readColor('data-secondary', '#6A5A8A');
    const GLYPHS = 'ᚐᚅᚒ';
    ...
    function draw() {
        ...
        for(let i=0;i<7;i++){ const seed=i*0.618; const x=width*((seed*461%1)); const y=height*(0.35+(seed*227%1)*0.35); const pulse=0.5+0.5*Math.sin(t*0.8+seed*30); const g=ctx.createRadialGradient(x,y,0,x,y,40+pulse*30); g.addColorStop(0,"rgba("+P.r+","+P.g+","+P.b+","+(0.14+0.08*pulse)+")"); g.addColorStop(1,"rgba("+P.r+","+P.g+","+P.b+",0)"); ctx.fillStyle=g; ctx.beginPath(); ctx.arc(x,y,40+pulse*30,0,Math.PI*2); ctx.fill(); } for(let i=0;i<16;i++){ const seed=i*0.618; const x=((seed*577+t*(3+(i%3)))%1)*width; const y=height*(0.6+(seed*197%1)*0.3); ctx.fillStyle="rgba("+S.r+","+S.g+","+S.b+",0.05)"; ctx.beginPath(); ctx.ellipse(x,y,50,9,0,0,Math.PI*2); ctx.fill(); }
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=annwn`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'annwn', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/annwn/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/annwn/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/annwn/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/annwn/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/annwn/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'annwn';
  const siteName = 'Annwn';
