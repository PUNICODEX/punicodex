...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'tartaros', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/tartaros/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'tartaros';
  const siteName = 'Tártaros';
