...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.ISHTAR_API_BASE || 'http://localhost:3456'; // Set window.ISHTAR_API_BASE in HTML for production
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots?site=ishtar`);
...
function openModal(slotId) {
  console.log('[PUNYCODEX:ishtar] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:ishtar] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:ishtar] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:ishtar] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:ishtar] Slot clicked:', slotId);
  openModal(slotId);
});
  ...
  try {
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/sites/ishtar/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/sites/ishtar/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/sites/ishtar/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
