...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'ishtar', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/ishtar/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'ishtar';
  const siteName = 'Ištar';
