...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'muspellheimr', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/muspellheimr/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'muspellheimr';
  const siteName = 'Muspellheimr';
