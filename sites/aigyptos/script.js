...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'aigyptos', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/aigyptos/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'aigyptos';
  const siteName = 'Aígyptos';
