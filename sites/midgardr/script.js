...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'midgardr', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/midgardr/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'midgardr';
  const siteName = 'Miðgarðr';
