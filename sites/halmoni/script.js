/**
 * Halmoni, the Grandmother Spirit — Earth Grandmother
 * Gentle green particles rise like breath from a dark earth, forming slow grandmotherly forms.
 */

(function () {
  'use strict';

  const canvas = document.getElementById('halmoni-canvas');
  if (!canvas) return;
  ...
  const particles = []; for (let i = 0; i < 80; i++) particles.push({ x: Math.random(), y: Math.random(), r: 1 + Math.random() * 2, speed: 0.1 + Math.random() * 0.3, life: Math.random() });
  let t = 0;
  function draw() {
    if (reduced) return;
    ctx.clearRect(0, 0, width, height);
    ctx.globalAlpha = 0.5; ctx.fillStyle = '#8fb080';
    for (const p of particles) {
      let y = (p.y - t * p.speed * 0.0003) % 1; if (y < 0) y += 1;
      const x = p.x * width + Math.sin(t * 0.0005 + p.life * 10) * 20;
      ctx.beginPath(); ctx.arc(x, y * height, p.r, 0, Math.PI * 2); ctx.fill();
    }
    ctx.globalAlpha = 1;
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=halmoni`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'halmoni', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/halmoni/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/halmoni/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/halmoni/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/halmoni/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/halmoni/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'halmoni';
  const siteName = 'Halmoni';
