...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'athenai', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/athenai/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'athenai';
  const siteName = 'Athēnai';
