...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'ganesha', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/ganesha/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'ganesha';
  const siteName = 'Gaṇeśa';
