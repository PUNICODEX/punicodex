...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.SHU_API_BASE || 'http://localhost:3456'; // Set window.SHU_API_BASE in HTML for production
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots?site=shu`);
...
function openModal(slotId) {
  console.log('[PUNYCODEX:shu] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:shu] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:shu] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:shu] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:shu] Slot clicked:', slotId);
  openModal(slotId);
});
  ...
  try {
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/sites/shu/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/sites/shu/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/sites/shu/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
