...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'anat', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/anat/dashboard/?token=${data.token}`;
      }
    } else {
