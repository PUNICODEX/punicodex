...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'shiva', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/shiva/dashboard/?token=${data.token}`;
      }
    } else {
