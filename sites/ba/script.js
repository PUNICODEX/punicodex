...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.BA_API_BASE || 'http://localhost:3456'; // Set window.BA_API_BASE in HTML for production
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots?site=ba`);
...
function openModal(slotId) {
  console.log('[PUNYCODEX:ba] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:ba] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:ba] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:ba] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:ba] Slot clicked:', slotId);
  openModal(slotId);
});
  ...
  try {
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/sites/ba/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/sites/ba/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/sites/ba/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
