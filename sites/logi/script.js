// Logi — The Wildfire (bespoke hero effect)
(function() {
    'use strict';
    ...
    const S = readColor('data-secondary', '#8A3A1E');
    const GLYPHS = 'ᛚᚢᚴᛁ';
    ...
    function draw() {
        ...
        for(let i=0;i<9;i++){ const x=width*(0.06+i*0.11); const h=height*(0.18+0.08*Math.sin(t*2.8+i*1.7)); const lean=18*Math.sin(t*1.6+i); ctx.strokeStyle="rgba("+P.r+","+P.g+","+P.b+","+(0.2+0.12*Math.abs(Math.sin(t*2.2+i)))+")"; ctx.lineWidth=4; ctx.beginPath(); ctx.moveTo(x,height*0.94); ctx.bezierCurveTo(x+lean*0.3,height*0.94-h*0.4,x+lean,height*0.94-h*0.75,x+lean*1.1,height*0.94-h); ctx.stroke(); ctx.strokeStyle="rgba(255,214,140,"+(0.1+0.08*Math.abs(Math.sin(t*3+i)))+")"; ctx.lineWidth=1.4; ctx.beginPath(); ctx.moveTo(x,height*0.94); ctx.bezierCurveTo(x+lean*0.3,height*0.94-h*0.35,x+lean*0.7,height*0.94-h*0.6,x+lean*0.8,height*0.94-h*0.8); ctx.stroke(); }
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=logi`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'logi', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/logi/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/logi/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/logi/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/logi/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/logi/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
