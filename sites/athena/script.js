...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.ATHENA_API_BASE || 'http://localhost:3456'; // Set window.ATHENA_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:athena] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:athena] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:athena] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:athena] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:athena] Slot clicked:', slotId);
  openModal(slotId);
});
