...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'enlil', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/enlil/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'enlil';
  const siteName = 'Enlīl';
