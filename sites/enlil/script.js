...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.ENLIL_API_BASE || 'http://localhost:3456'; // Set window.ENLIL_API_BASE in HTML for production
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots?site=enlil`);
...
function openModal(slotId) {
  console.log('[PUNYCODEX:enlil] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:enlil] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:enlil] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:enlil] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:enlil] Slot clicked:', slotId);
  openModal(slotId);
});
  ...
  try {
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/sites/enlil/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/sites/enlil/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/sites/enlil/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
