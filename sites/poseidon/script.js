...
(function() {
    ...
    // ============================
      ...
      console.log('[poseidon] Canvas ocean-canvas not present on this page');
    }
...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.POSEIDON_API_BASE || 'http://localhost:3456'; // Set window.POSEIDON_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:poseidon] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:poseidon] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:poseidon] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:poseidon] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:poseidon] Slot clicked:', slotId);
  openModal(slotId);
});
