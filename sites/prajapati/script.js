...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'prajapati', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/prajapati/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'prajapati';
  const siteName = 'Prajāpati';
