...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'asherah', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/asherah/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'asherah';
  const siteName = 'ꜥAsherah';
