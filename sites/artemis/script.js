...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots?site=artemis`);
...
function openModal(slotId) {
  console.log('[PUNYCODEX:artemis] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:artemis] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:artemis] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:artemis] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:artemis] Slot clicked:', slotId);
  openModal(slotId);
});
  ...
  try {
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/sites/artemis/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/sites/artemis/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/sites/artemis/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
