...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'artemis', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/artemis/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'artemis';
  const siteName = 'Ártemis';
