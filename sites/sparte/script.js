...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'sparte', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/sparte/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'sparte';
  const siteName = 'Spártē';
