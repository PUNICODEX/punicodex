...
(function() {
    'use strict';
    const canvas = document.getElementById('rhinegold-canvas');
    ...
    const S = readColor('data-secondary', '#4A6A8A');
    const GLYPHS = 'ᛅᚾᛏᚢᛅᚱᛁ';
    ...
    function draw() {
        ...
        for(let i=0;i<26;i++){ const seed=i*0.618; const x=((seed*733%1))*width; const y=height*0.25+((seed*311+t*(0.03+(i%4)*0.008))%1)*height*0.65; const a=0.06+0.1*Math.abs(Math.sin(t*0.8+i)); ctx.fillStyle="rgba("+P.r+","+P.g+","+P.b+","+a+")"; ctx.beginPath(); ctx.arc(x,y,1.2+(i%3)*0.6,0,Math.PI*2); ctx.fill(); } for(let i=0;i<5;i++){ const y=height*(0.3+i*0.14); ctx.strokeStyle="rgba("+S.r+","+S.g+","+S.b+","+(0.12-i*0.015)+0.03*Math.sin(t*0.7+i)+")"; ctx.lineWidth=1.2; ctx.beginPath(); for(let x=0;x<=width;x+=20){ const yy=y+Math.sin(x*0.01+t+i)*8; if(x===0)ctx.moveTo(x,yy); else ctx.lineTo(x,yy);} ctx.stroke(); }
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=andvari`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'andvari', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/andvari/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/andvari/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/andvari/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/andvari/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/andvari/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'andvari';
  const siteName = 'Andvari';
