...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'delphoi', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/delphoi/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'delphoi';
  const siteName = 'Delphoí';
