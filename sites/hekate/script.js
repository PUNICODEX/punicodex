(function() {
    ...
    window.addEventListener('resize', resize);
      ...
      console.log('[hekate] Canvas crossroads-canvas not present on this page');
    }
...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.HEKATE_API_BASE || 'http://localhost:3456'; // Set window.HEKATE_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:hekate] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:hekate] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:hekate] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:hekate] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:hekate] Slot clicked:', slotId);
  openModal(slotId);
});
