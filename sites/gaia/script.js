...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.GAIA_API_BASE || 'http://localhost:3456'; // Set window.GAIA_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:gaia] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:gaia] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:gaia] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:gaia] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:gaia] Slot clicked:', slotId);
  openModal(slotId);
});
