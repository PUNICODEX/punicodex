...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'typhon', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/typhon/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'typhon';
  const siteName = 'Typhōn';
