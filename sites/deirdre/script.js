// Deirdre — The Sorrows of the Sea (bespoke hero effect)
(function() {
    'use strict';
    const canvas = document.getElementById('sorrowssea-canvas');
    const ctx = canvas ? canvas.getContext('2d') : null;
    if (!ctx) return;
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    function hexToRgb(hex){const n=parseInt(hex.replace('#',''),16);return{r:(n>>16)&255,g:(n>>8)&255,b:n&255};}
    function readColor(attr, fb){const v=canvas.getAttribute(attr);return v&&v.startsWith('#')?hexToRgb(v):hexToRgb(fb);}
    const P = readColor('data-primary', '#7A9AB8');
    const S = readColor('data-secondary', '#3A4A5A');
    const GLYPHS = 'ᚇᚓᚔᚏ';
    ...
    function draw() {
        ...
        for(let i=0;i<5;i++){ const y=height*(0.35+i*0.11); ctx.strokeStyle="rgba("+P.r+","+P.g+","+P.b+","+(0.14-i*0.02)+0.03*Math.sin(t*0.7+i)+")"; ctx.lineWidth=1.4; ctx.beginPath(); for(let x=0;x<=width;x+=18){ const yy=y+Math.sin(x*0.01+t+i)*9; if(x===0)ctx.moveTo(x,yy); else ctx.lineTo(x,yy);} ctx.stroke(); } for(let i=0;i<3;i++){ const seed=i*0.618; const x=width*(0.25+(seed*461%1)*0.5); const y=height*(0.3+(seed*227%1)*0.2); const pulse=0.5+0.5*Math.sin(t*0.8+seed*30); const g=ctx.createRadialGradient(x,y,0,x,y,30+pulse*20); g.addColorStop(0,"rgba("+P.r+","+P.g+","+P.b+","+(0.12+0.1*pulse)+")"); g.addColorStop(1,"rgba("+P.r+","+P.g+","+P.b+",0)"); ctx.fillStyle=g; ctx.beginPath(); ctx.arc(x,y,30+pulse*20,0,Math.PI*2); ctx.fill(); }
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=deirdre`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'deirdre', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/deirdre/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/deirdre/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/deirdre/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/deirdre/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/deirdre/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'deirdre';
  const siteName = 'Deirdre';
