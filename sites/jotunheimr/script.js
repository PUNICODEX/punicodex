...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'jotunheimr', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/jotunheimr/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'jotunheimr';
  const siteName = 'Jötunheimr';
