// Ginnungagap — The Yawning Void (bespoke hero effect)
(function() {
    'use strict';
    const canvas = document.getElementById('yawningvoid-canvas');
    const ctx = canvas ? canvas.getContext('2d') : null;
    if (!ctx) return;
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    function hexToRgb(hex){const n=parseInt(hex.replace('#',''),16);return{r:(n>>16)&255,g:(n>>8)&255,b:n&255};}
    function readColor(attr, fb){const v=canvas.getAttribute(attr);return v&&v.startsWith('#')?hexToRgb(v):hexToRgb(fb);}
    const P = readColor('data-primary', '#7A8AA8');
    const S = readColor('data-secondary', '#D8863A');
    const GLYPHS = 'ᚴᛁᚾᚢᚾᚴᛅ';
    ...
    function draw() {
        ...
        for(let i=0;i<16;i++){ const seed=i*0.618; const warm=i%2===0; const c=warm?S:P; const x=((seed*431+t*(0.015+(i%3)*0.006))%1)*width; const y=warm?height*(0.1+(seed*197%1)*0.4):height*(0.5+(seed*197%1)*0.4); const a=0.05+0.09*Math.abs(Math.sin(t*0.8+i)); ctx.fillStyle="rgba("+c.r+","+c.g+","+c.b+","+a+")"; ctx.beginPath(); ctx.arc(x,y,1.6+(i%3)*0.8,0,Math.PI*2); ctx.fill(); } const midY=height*0.5; for(let i=0;i<10;i++){ const x=width*(0.1+i*0.09); const mix=Math.sin(t*0.6+i)*20; ctx.fillStyle="rgba("+(P.r*0.6+S.r*0.4)+","+(P.g*0.6+S.g*0.4)+","+(P.b*0.6+S.b*0.4)+","+(0.05+0.05*Math.sin(t+i))+")"; ctx.beginPath(); ctx.arc(x+mix,midY+Math.sin(t*0.5+i)*14,2,0,Math.PI*2); ctx.fill(); }
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=ginnungagap`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'ginnungagap', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/ginnungagap/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/ginnungagap/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/ginnungagap/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/ginnungagap/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/ginnungagap/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'ginnungagap';
  const siteName = 'Ginnungagap';
