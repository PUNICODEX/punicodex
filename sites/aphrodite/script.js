...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.APHRODITE_API_BASE || 'http://localhost:3456'; // Set window.APHRODITE_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:aphrodite] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:aphrodite] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:aphrodite] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:aphrodite] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:aphrodite] Slot clicked:', slotId);
  openModal(slotId);
});
