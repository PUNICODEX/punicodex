...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.MAA_API_BASE || 'http://localhost:3456'; // Set window.MAA_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:maa] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:maa] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:maa] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:maa] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:maa] Slot clicked:', slotId);
  openModal(slotId);
});
