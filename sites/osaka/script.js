...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'osaka', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/osaka/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'osaka';
  const siteName = 'Ōsaka';
