// Ceridwen — The Cauldron of Awen (bespoke hero effect)
(function() {
    'use strict';
    const canvas = document.getElementById('awen-canvas');
    const ctx = canvas ? canvas.getContext('2d') : null;
    if (!ctx) return;
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    function hexToRgb(hex){const n=parseInt(hex.replace('#',''),16);return{r:(n>>16)&255,g:(n>>8)&255,b:n&255};}
    function readColor(attr, fb){const v=canvas.getAttribute(attr);return v&&v.startsWith('#')?hexToRgb(v):hexToRgb(fb);}
    const P = readColor('data-primary', '#8AB8A0');
    const S = readColor('data-secondary', '#4A6A58');
    const GLYPHS = 'ᚉᚓᚏ';
    ...
    function draw() {
        ...
        const cx=width/2, rimY=height*0.6; ctx.strokeStyle="rgba("+P.r+","+P.g+","+P.b+",0.35)"; ctx.lineWidth=2; ctx.beginPath(); ctx.ellipse(cx,rimY,width*0.15,height*0.05,0,0,Math.PI*2); ctx.stroke(); for(let i=0;i<3;i++){ const a=(i/3)*Math.PI*2-t*0.4; const x=cx+Math.cos(a)*width*0.11, y=rimY+Math.sin(a)*height*0.04; const pulse=0.5+0.5*Math.sin(t*1.6-i*2.1); const g=ctx.createRadialGradient(x,y,0,x,y,14+pulse*12); g.addColorStop(0,"rgba(240,230,160,"+(0.3+0.3*pulse)+")"); g.addColorStop(1,"rgba(240,230,160,0)"); ctx.fillStyle=g; ctx.beginPath(); ctx.arc(x,y,14+pulse*12,0,Math.PI*2); ctx.fill(); } for(let i=0;i<8;i++){ const seed=i*0.618; const x=cx-width*0.1+(seed*311%1)*width*0.2; const y=rimY-((seed*197+t*0.05)%1)*height*0.3; ctx.strokeStyle="rgba("+S.r+","+S.g+","+S.b+","+(0.06+0.05*Math.sin(t+i))+")"; ctx.lineWidth=2; ctx.beginPath(); ctx.arc(x,y,4+3*Math.sin(t+i),0,Math.PI*1.3); ctx.stroke(); }
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=ceridwen`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'ceridwen', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/ceridwen/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/ceridwen/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/ceridwen/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/ceridwen/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/ceridwen/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'ceridwen';
  const siteName = 'Ceridwen';
