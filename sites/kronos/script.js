...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'kronos', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/kronos/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'kronos';
  const siteName = 'Krónos';
