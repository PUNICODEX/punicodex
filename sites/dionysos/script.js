...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'dionysos', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/dionysos/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'dionysos';
  const siteName = 'Diónysos';
