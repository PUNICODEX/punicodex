...
(function() {
    'use strict';
    const canvas = document.getElementById('skaldharp-canvas');
    ...
    const S = readColor('data-secondary', '#8A7A5A');
    const GLYPHS = 'ᛒᚱᛅᚴᛁ';
    ...
    function draw() {
        ...
        const cx=width/2; for(let i=0;i<7;i++){ const x=cx-width*0.18+i*width*0.06; const vib=Math.sin(t*3+i*1.2)*3; ctx.strokeStyle="rgba("+P.r+","+P.g+","+P.b+","+(0.14+0.08*Math.abs(Math.sin(t*2+i)))+")"; ctx.lineWidth=1.2; ctx.beginPath(); ctx.moveTo(x,height*0.25); ctx.bezierCurveTo(x+vib,height*0.4,x-vib,height*0.6,x+vib*0.5,height*0.78); ctx.stroke(); } for(let i=0;i<7;i++){ const x=cx-width*0.18+i*width*0.06; const pluck=Math.max(0,Math.sin(t*2-i*0.9))*0.3; ctx.fillStyle="rgba(232,214,140,"+pluck+")"; ctx.beginPath(); ctx.arc(x,height*0.52,2.4,0,Math.PI*2); ctx.fill(); }
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=bragi`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'bragi', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/bragi/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/bragi/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/bragi/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/bragi/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/bragi/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'bragi';
  const siteName = 'Bragi';
