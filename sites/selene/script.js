...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'selene', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/selene/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'selene';
  const siteName = 'Selēnē';
