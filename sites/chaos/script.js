...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.CHAOS_API_BASE || 'http://localhost:3456'; // Set window.CHAOS_API_BASE in HTML for production
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots?site=chaos`);
...
function openModal(slotId) {
  console.log('[PUNYCODEX:chaos] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:chaos] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:chaos] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:chaos] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:chaos] Slot clicked:', slotId);
  openModal(slotId);
});
  ...
  try {
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/sites/chaos/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/sites/chaos/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/sites/chaos/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
