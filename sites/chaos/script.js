...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'chaos', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/chaos/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'chaos';
  const siteName = 'Cháos';
