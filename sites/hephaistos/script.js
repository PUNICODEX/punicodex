...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.HEPHAISTOS_API_BASE || 'http://localhost:3456'; // Set window.HEPHAISTOS_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:hephaistos] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:hephaistos] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:hephaistos] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:hephaistos] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:hephaistos] Slot clicked:', slotId);
  openModal(slotId);
});
