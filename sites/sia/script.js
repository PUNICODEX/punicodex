...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.SIA_API_BASE || 'http://localhost:3456'; // Set window.SIA_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:sia] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:sia] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:sia] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:sia] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:sia] Slot clicked:', slotId);
  openModal(slotId);
});
