...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.APOLLON_API_BASE || 'http://localhost:3456'; // Set window.APOLLON_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:apollon] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:apollon] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:apollon] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:apollon] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:apollon] Slot clicked:', slotId);
  openModal(slotId);
});
