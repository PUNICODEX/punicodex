...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'medousa', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/medousa/dashboard/?token=${data.token}`;
      }
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'medousa';
  const siteName = 'Médousa';
