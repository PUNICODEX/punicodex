...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.ATLAS_API_BASE || 'http://localhost:3456'; // Set window.ATLAS_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:atlas] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:atlas] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:atlas] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:atlas] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:atlas] Slot clicked:', slotId);
  openModal(slotId);
});
