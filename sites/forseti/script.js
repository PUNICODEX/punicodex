// Forseti — The Hall of Glitnir (bespoke hero effect)
(function() {
    'use strict';
    const canvas = document.getElementById('glitnir-canvas');
    const ctx = canvas ? canvas.getContext('2d') : null;
    if (!ctx) return;
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    function hexToRgb(hex){const n=parseInt(hex.replace('#',''),16);return{r:(n>>16)&255,g:(n>>8)&255,b:n&255};}
    function readColor(attr, fb){const v=canvas.getAttribute(attr);return v&&v.startsWith('#')?hexToRgb(v):hexToRgb(fb);}
    const P = readColor('data-primary', '#D8D4C8');
    const S = readColor('data-secondary', '#8A8A7A');
    const GLYPHS = 'ᚠᚢᚱᛋᛖᛏᛁ';
    ...
    function draw() {
        ...
        const cx=width/2, roofY=height*0.34, baseY=height*0.72; ctx.strokeStyle="rgba("+P.r+","+P.g+","+P.b+",0.3)"; ctx.lineWidth=1.8; ctx.beginPath(); ctx.moveTo(cx-width*0.2,baseY); ctx.lineTo(cx,roofY); ctx.lineTo(cx+width*0.2,baseY); ctx.stroke(); for(let i=0;i<5;i++){ const x=cx-width*0.16+i*width*0.08; const shimmer=0.1+0.1*Math.abs(Math.sin(t*1.1+i)); ctx.strokeStyle="rgba("+P.r+","+P.g+","+P.b+","+shimmer+")"; ctx.beginPath(); ctx.moveTo(x,baseY); ctx.lineTo(x,roofY+(baseY-roofY)*Math.abs(x-cx)/(width*0.2)+8); ctx.stroke(); } const beamW=width*0.3; ctx.strokeStyle="rgba("+S.r+","+S.g+","+S.b+",0.3)"; ctx.beginPath(); ctx.moveTo(cx-beamW/2,baseY); ctx.lineTo(cx+beamW/2,baseY); ctx.stroke(); const scale=Math.sin(t*0.4)*6; ctx.lineWidth=1.4; ctx.beginPath(); ctx.moveTo(cx-40,baseY-40-scale); ctx.lineTo(cx+40,baseY-40+scale); ctx.stroke();
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=forseti`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'forseti', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/forseti/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/forseti/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/forseti/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/forseti/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/forseti/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
...
(function initPatronSystem() {
  ...
  const templeId = 'forseti';
  const siteName = 'Forseti';
