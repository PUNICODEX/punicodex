...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.PERSEPHONE_API_BASE || 'http://localhost:3456'; // Set window.PERSEPHONE_API_BASE in HTML for production
...
function openModal(slotId) {
  console.log('[PUNYCODEX:persephone] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:persephone] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:persephone] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:persephone] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:persephone] Slot clicked:', slotId);
  openModal(slotId);
});
