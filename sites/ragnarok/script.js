...
(function() {
    ...
    // ============================
      ...
      console.log('[ragnarok] Canvas fire-canvas not present on this page');
    }
...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.RAGNAROK_API_BASE || 'http://localhost:3456'; // Set window.RAGNAROK_API_BASE in HTML for production
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots?site=ragnarok`);
...
function openModal(slotId) {
  console.log('[PUNYCODEX:ragnarok] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:ragnarok] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:ragnarok] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:ragnarok] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:ragnarok] Slot clicked:', slotId);
  openModal(slotId);
});
  ...
  try {
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/sites/ragnarok/dashboard/?token=${token}`;
      } else {
        showStep('3');
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/sites/ragnarok/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
