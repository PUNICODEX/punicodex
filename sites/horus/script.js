...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'horus', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/sites/horus/dashboard/?token=${data.token}`;
      }
    } else {
