...
(function() {
    ...
    // ============================
      ...
      console.log('[zeus] Canvas lightning-canvas not present on this page');
    }
...
// ========== BOOKING SYSTEM ==========
const API_BASE = window.ZEUS_API_BASE || 'http://localhost:3456'; // Set window.ZEUS_API_BASE in HTML for production
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots?site=zeus`);
...
function openModal(slotId) {
  console.log('[PUNYCODEX:zeus] openModal called:', slotId);
  try {
    ...
    if (!slot) {
      ...
      if (!slotEl) { console.log('[PUNYCODEX:zeus] No slotEl found'); return; }
    ...
    console.log('[PUNYCODEX:zeus] Modal opened for slot', slotId);
  } catch (err) {
    console.error('[PUNYCODEX:zeus] openModal error:', err);
  }
}
...
document.addEventListener('click', (e) => {
  ...
  console.log('[PUNYCODEX:zeus] Slot clicked:', slotId);
  openModal(slotId);
});
  ...
  try {
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/sites/zeus/dashboard/?token=${token}`;
      } else {
        showStep('3');
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/sites/zeus/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
