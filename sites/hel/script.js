// Hel — The Gates of Éljúðnir (bespoke hero effect)
(function() {
    'use strict';
    const canvas = document.getElementById('nilfheim-canvas');
    const ctx = canvas ? canvas.getContext('2d') : null;
    if (!ctx) return;
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    function hexToRgb(hex){const n=parseInt(hex.replace('#',''),16);return{r:(n>>16)&255,g:(n>>8)&255,b:n&255};}
    function readColor(attr, fb){const v=canvas.getAttribute(attr);return v&&v.startsWith('#')?hexToRgb(v):hexToRgb(fb);}
    const P = readColor('data-primary', '#6A7B8C');
    const S = readColor('data-secondary', '#3E4A56');
    const GLYPHS = 'ᚺᛖᛚ';
    ...
    function draw() {
        ...
        const gateX=width/2, gateY=height*0.38; ctx.strokeStyle="rgba("+P.r+","+P.g+","+P.b+",0.22)"; ctx.lineWidth=1.4; ctx.beginPath(); ctx.moveTo(gateX-width*0.12,height); ctx.lineTo(gateX-width*0.12,gateY); ctx.quadraticCurveTo(gateX,gateY-height*0.1,gateX+width*0.12,gateY); ctx.lineTo(gateX+width*0.12,height); ctx.stroke(); for(let i=0;i<20;i++){ const seed=i*0.618; const x=((seed*777+t*(6+(i%4)*2))%1)*width; const y=height*0.55+Math.sin(seed*97+t*0.4+i)*height*0.06+ (i%5)*height*0.05; ctx.fillStyle="rgba("+S.r+","+S.g+","+S.b+","+(0.03+0.04*Math.abs(Math.sin(t*0.7+i)))+")"; ctx.beginPath(); ctx.ellipse(x,y,60+20*(i%3),10+4*(i%2),0,0,Math.PI*2); ctx.fill(); }
...
async function loadSlots() {
  try {
    const res = await fetch(`${API_BASE}/api/slots/?site=hel`);
...
async function validateDiscountCode() {
  ...
  try {
    const payload = { code, temple: 'hel', leaseMonths: currentLeaseMonths };
  ...
  try {
    ...
    if (data.status === 'pending_application') {
      ...
      if (els.dashboardLink) {
        els.dashboardLink.href = `${API_BASE}/hel/dashboard/?token=${data.token}`;
      }
    } else {
...
async function handleReturnFromStripe() {
  ...
  try {
    ...
    if (renewed && booking.status === 'live') {
      ...
      els.dashboardLink.href = `${API_BASE}/hel/dashboard/?token=${token}`;
      await loadSlots();
      return;
    }
    ...
    if (paid && (booking.status === 'pending_upload' || booking.status === 'live')) {
      ...
      if (booking.status === 'pending_upload') {
        ...
        if (dashLink2) dashLink2.href = `${API_BASE}/hel/dashboard/?token=${token}`;
      } else {
        showStep('3');
        els.dashboardLink.href = `${API_BASE}/hel/dashboard/?token=${token}`;
      }
...
els.submitUpload.addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      showStep('3');
      els.dashboardLink.href = `${API_BASE}/hel/dashboard/?token=${currentBooking.analytics_token}`;
    } else {
